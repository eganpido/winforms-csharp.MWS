...
{
    public partial class RepAdhocReceivingPDFView : Form
    {
        public DateTime dateStart;
        public DateTime dateEnd;
        public int branchId;
        public string branchName;
        public RepAdhocReceivingPDFView(DateTime startDate, DateTime endDate, int _branchId)
        {
            InitializeComponent();
        ...
        public void PrintReport()
        {
            try
            {
                ...
                var fileName = "Ad-hocReceivingReport" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".pdf";
                ...
                var receivingItems = repInventoryController.AdhocReceivingReport(dateStart, dateEnd, branchId);

                PdfPTable tableReceivingItems = new PdfPTable(4);
                tableReceivingItems.SetWidths(new float[] { 100f, 100f, 100f, 100f });
                tableReceivingItems.WidthPercentage = 100;
                ...
                if (receivingItems.Any())
                {
                    ...
                    tableReceivingItems.AddCell(new PdfPCell(new Phrase("BARCODE", fontTimesNewRoman10Bold)) { PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });
                    tableReceivingItems.AddCell(new PdfPCell(new Phrase("ITEM", fontTimesNewRoman10Bold)) { PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });
                    tableReceivingItems.AddCell(new PdfPCell(new Phrase("SIZE", fontTimesNewRoman10Bold)) { PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });
                    tableReceivingItems.AddCell(new PdfPCell(new Phrase("CLASSIFICATION", fontTimesNewRoman10Bold)) { PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });

                    foreach (var item in receivingItems)
                    {
                        tableReceivingItems.AddCell(new PdfPCell(new Phrase(item.ReceivingBarcode, fontTimesNewRoman10)) { Border = 0, PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });
                        tableReceivingItems.AddCell(new PdfPCell(new Phrase(item.Item, fontTimesNewRoman10)) { Border = 0, PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });
                        tableReceivingItems.AddCell(new PdfPCell(new Phrase(item.Size, fontTimesNewRoman10)) { Border = 0, PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });
                        tableReceivingItems.AddCell(new PdfPCell(new Phrase(item.Classification, fontTimesNewRoman10)) { Border = 0, PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });
                        count++;
                    ...
                    tableReceivingItems.AddCell(new PdfPCell(new Phrase("TOTALS - SPICY : " + spicy.ToString("#,##0") + " CLASSIC : " + classic.ToString("#,##0") + " COUNT : " + count.ToString("#,##0"), fontTimesNewRoman10Bold)) { Colspan = 4, PaddingLeft = 3f, PaddingRight = 3f, PaddingTop = 3f, PaddingBottom = 5f, HorizontalAlignment = 1 });
                }

                document.Add(tableReceivingItems);
                document.Close();
        ...
        class ConfigureHeaderFooter : PdfPageEventHelper
        {
            ...
            public override void OnEndPage(PdfWriter writer, Document document)
            {
                ...
                String documentTitle = "Ad-hoc Receiving Report";
